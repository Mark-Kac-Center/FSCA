function SFCHilbert2DCase_scales(filename, isPad)

% program oblicza Hilberta dla roznych skal
% isPad = true dla macierz niekwadratowej
...
%subplot(1,2,2); imagesc(V);size(V)
...
order = [5:log(numel(V))/log(4)];
    
for j=1:length(order)
    
    [rD, rI] = hilbertCurve2D_order(V,order(j));
 
    %subplot(2,2,4);plot(1:length(rD),rD);title('Hilbert'); axis tight
    HLT = rD;
    ...
    baseFileName_order = [baseFileName, '_order', num2str(order(j))];
    
    hLTfilename = sprintf('HilbertLT%s.csv', baseFileName_order);
    hVOfilename = sprintf('HilbertVO%s.csv', baseFileName_order);
